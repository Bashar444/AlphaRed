    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex gap-2">
                <Input value={newTitle} onChange={(e) => setNewTitle(e.target.value)} placeholder="Add a new to-do..." className="bg-slate-800 border-slate-700 text-white max-w-lg" onKeyDown={(e) => e.key === "Enter" && addTodo()} />
            ...
            {data.length === 0 ? (
                ...
                <div className="space-y-6">
                    <Card className="bg-slate-900 border-slate-800">
                        <CardHeader><CardTitle className="text-white text-sm">Pending ({pending.length})</CardTitle></CardHeader>
                        <CardContent className="space-y-2">
                            {pending.map((t) => (
                                <div key={t.id} className="flex items-center gap-3 p-2 rounded-lg hover:bg-slate-800/50 cursor-pointer" onClick={() => toggleTodo(t.id)}>
                    ...
                    {done.length > 0 && (
                        <Card className="bg-slate-900 border-slate-800">
                            ...
                            <CardContent className="space-y-2">
                                {done.map((t) => (
                                    <div key={t.id} className="flex items-center gap-3 p-2 rounded-lg hover:bg-slate-800/50 cursor-pointer opacity-60" onClick={() => toggleTodo(t.id)}>
                                        <div className="w-5 h-5 rounded border-2 border-emerald-500 bg-emerald-500/20 flex-shrink-0 flex items-center justify-center">
